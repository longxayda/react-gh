...
const CardItem = ({ title, price, img }) => (
  <div className="card border-0" style={{ minHeight: "200px" }}>
    <div className="card-body">
      <img className="mb-3" style={{ width: "130px",height:'150px', objectFit: "cover" }} src={img} alt="no"></img>
...
const PilowMist = () => {
  ...
  const fetchProducts = async (sortCriteria) => {
    let apiUrl = `http://171.244.188.157:443/collections/BRUME%20D'OREILLER`;
        ...
        <div className="row mt-3">
          ...
          {products.data?.map((product) => (
            <div key={product._id} className="col-6 col-sm-6 col-md-4 col-lg-3 mb-4">
              <NavLink to={`/product/${product._id}`} style={{ textDecoration: "none" }}>
                <CardItem
                  title={product.name}
